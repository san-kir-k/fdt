#pragma once

#include <cstdint>

#include <arrow/api.h>
#include <arrow/type_traits.h>

#include "conversion/dispatch/soa2aos.h"
#include "pretty_type_traits.h"
#include "aos/aos.h"

#include <iostream>


inline std::shared_ptr<AoS> SoA2AoS(std::shared_ptr<arrow::RecordBatch> record_batch)
{
    std::shared_ptr<AoS> aos(new AoS(record_batch->schema()));
    aos->PrepareSelf(record_batch);

    uint64_t datalen = record_batch->num_rows();
    uint64_t outsz = aos->GetStructSize();
    auto fields = aos->GetFields();

    static const void* gotoTable[] = {&&transpose1, &&transpose2, &&transpose3, &&transpose4};
    uint64_t i = 0;
    uint64_t offset = 0;

    while (i < fields.size())
    {
        uint64_t rbound = 1;
        for (; rbound < 4 && i + rbound < fields.size(); ++rbound)
        {
            if (GetCTypeSize(fields[i + rbound]->type()) != GetCTypeSize(fields[i + rbound - 1]->type()))
            {
                break;
            }
        }

        uint64_t insz = GetCTypeSize(fields[i]->type());

        goto *gotoTable[rbound - 1];

        transpose4:
        {
            auto p1 = record_batch->GetColumnByName(fields[i]->name());
            auto p2 = record_batch->GetColumnByName(fields[i + 1]->name());
            auto p3 = record_batch->GetColumnByName(fields[i + 2]->name());
            auto p4 = record_batch->GetColumnByName(fields[i + 3]->name());

            SoA2AoSx4(p1, p2, p3, p4, insz, aos->GetBuffer() + offset, outsz, datalen);
            goto update;
        }
        transpose3:
        {
            auto p1 = record_batch->GetColumnByName(fields[i]->name());
            auto p2 = record_batch->GetColumnByName(fields[i + 1]->name());
            auto p3 = record_batch->GetColumnByName(fields[i + 2]->name());

            SoA2AoSx3(p1, p2, p3, insz, aos->GetBuffer() + offset, outsz, datalen);
            goto update;
        }
        transpose2:
        {
            auto p1 = record_batch->GetColumnByName(fields[i]->name());
            auto p2 = record_batch->GetColumnByName(fields[i + 1]->name());

            SoA2AoSx2(p1, p2, insz, aos->GetBuffer() + offset, outsz, datalen);
            goto update;
        }
        transpose1:
        {
            auto p1 = record_batch->GetColumnByName(fields[i]->name());

            SoA2AoSx1(p1, insz, aos->GetBuffer() + offset, outsz, datalen);
            goto update;
        }
        update:
            offset += rbound * insz;
            i += rbound;
    }

    return aos;
}
