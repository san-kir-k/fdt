...
arrow::Status CreateAndFillRecordBatch(std::shared_ptr<arrow::RecordBatch>& record, uint64_t size, uint64_t cols, uint64_t str_len)
{
    ...
    record = arrow::RecordBatch::Make(schema, size, to_record);
    return arrow::Status::OK();
}
...
arrow::Status RunLongTests()
{
    ...
    for (uint64_t str_len: str_len_bounds)
    {
        ...
        for (uint64_t rows: rows_lens)
        {
            ...
            auto status = CreateAndFillRecordBatch(record, rows, cols_size, str_len);
            if (!status.ok())
            {
...
arrow::Status RunWideTests()
{
    ...
    for (uint64_t str_len: str_len_bounds)
    {
        ...
        for (uint64_t cols: cols_lens)
        {
            ...
            auto status = CreateAndFillRecordBatch(record, size, cols, str_len);
            if (!status.ok())
            {
                std::cerr << status.ToString() << std::endl;
                return status;
