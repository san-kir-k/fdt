...
def construct_run_func(field_types: list[str], types_map: dict) -> str:
    run_func_str: str = "void Run(uint8_t* src, SoA& dst, uint64_t size)\n{\n    std::vector<SoAField> table = {\n"
    
    for i, type in enumerate(field_types):
        run_func_str += f"        SoAField{{.ptr = reinterpret_cast<uint8_t*>(dst.f{i}), .size = sizeof({type})}},\n"
    run_func_str += "    };\n"
    
    run_func_str += f'    BENCHMARK_VOID("AoS -> SoA: Primitive static speed: ", size * (sizeof({base_types[0]}) * {types_map[base_types[0]]}'\
                    f' + sizeof({base_types[1]}) * {types_map[base_types[1]]} + sizeof({base_types[2]}) * {types_map[base_types[2]]}'\
                    f' + sizeof({base_types[3]}) * {types_map[base_types[3]]}), AoS2SoA, src, table, size);\n' + '}\n\n'
    return run_func_str
...
def construct_main_func(field_types: list[str], types_map: dict) -> str:
    ...
    main_func_str += f'    uint8_t* aos = new unsigned char[size * ('\
    ...
    main_func_str += '    Run(aos, soa, size);\n    delete[] aos;\n'
